...
public partial class SongDetailsPage : ContentPage
{
    ...
    /// <summary>
    /// Pobierz dane o utworze z Deezer na podstawie dostępnych informacji (np. tytuł, wykonawca) i przygotuj odtwarzacz.
    /// </summary>
    /// <returns></returns>
...
#pragma warning restore CS1998 
    ...
    /// <summary>
    /// Zdarzenie nawigacji WebView - przechwyć niestandardowy schemat app:// do komunikacji z JS (np. pokazywanie/ukrywanie kontrolek, reagowanie na gesty pinch).
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
        ...
        try
        {
            ...
            if (double.TryParse(posStr?.Trim('"'), out var pos))
            {
                if (pos == 0 || true)
                {
                    ...
                    {
                        const double rampUpDuration = 60.0;   // czas rozruchu w sekundach
                        const double minSpeedFactor = 0.2;    // startowa prędkość (20%)
                        ...
                        switch(_songEntity?.ScrollingStartFunction ?? "quad")
                        {
                            ...
                            case "quad":
                                ...
                                break;
                            case "cubic":
                                rampIntegral = minSpeedFactor + (1.0 - minSpeedFactor) / 4.0; // całka funkcji sześciennej po [0,1]: min + (1-min)/4
                                break;
                            case "log":
                        ...
                        double rampCorrection = rampUpDuration * (1.0 - rampIntegral); // ≈ 32 s
    ...
    /// <summary>
    /// Zdarzenie koniec przewijania - zatrzymaj auto-scroll i pokaż kontrolki
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
